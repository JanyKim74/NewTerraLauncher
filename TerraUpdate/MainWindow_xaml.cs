...
#pragma warning restore CS0649
        ...
        private void UpdateGameVersionDisplay()
        {
            try
            {
                string gameVersion = updateManager.GetCurrentGameVersion(); // 가장 정확한 방법!

                // 안전장치: 만약 null이거나 비어있으면 서버 정보에서 가져오기
                if (string.IsNullOrWhiteSpace(gameVersion) || gameVersion == "0.0.1")
                {
                    if (currentUpdateInfo?.CurrentUpdateIndex != null)
                    {
                        gameVersion = currentUpdateInfo.CurrentUpdateIndex;
                    }
                    else if (currentUpdateInfo?.UpdateIndex != null)
                    {
                        gameVersion = $"1.{currentUpdateInfo.UpdateIndex}";
                    }
                    else
                    {
                        gameVersion = "0.0.1";// 최후의 보루
                    }
                }
                ...
                Console.WriteLine($"게임 버전 표시: Game v{gameVersion}");
            }
            catch (Exception ex)
            {
